...
test('batch3 capability registry indexes agents, skills, scripts, and command aliases', () => {
  ...
  assert.equal(byId['agent:eoc_code_reviewer'].surface, 'recommended');
  assert.equal(byId['agent:eoc_code_reviewer'].maturity, 'stable');
  assert.equal(byId['agent:eoc_code_reviewer'].recommended, true);
  assert.equal(byId['agent:e2e-runner'].kind, 'verifier');
  assert.equal(byId['agent:e2e-runner'].surface, 'extended');
  ...
  assert.equal(byId['script:project-profile'].surface, 'recommended');
  assert.equal(byId['script:project-profile'].recommended, true);
  assert.equal(byId['skill:openspec-workflow'].kind, 'planner');
  assert.equal(byId['skill:openspec-workflow'].maturity, 'beta');
  assert.equal(byId['skill:tdd-workflow'].surface, 'recommended');
  assert.equal(byId['skill:tdd-workflow'].recommended, true);
