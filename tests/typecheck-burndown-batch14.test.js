...
test('batch14 converts workflow/scheduler/platform helpers to strict-safe and keeps quarantine aligned', () => {
  ...
  assert.ok(report.strict_checked >= 137, `expected strict_checked >= 137, got ${report.strict_checked}`);
  assert.ok(report.quarantined <= 56, `expected quarantined <= 56, got ${report.quarantined}`);
