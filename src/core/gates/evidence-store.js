...
/** @typedef {{ id?: string, created_at?: string, tags?: string[], meta?: Record<string, unknown> }} EvidenceExtra */
/** @typedef {{ id: string, type: string, producer: string, created_at: string, content: unknown, digest: string, tags: string[], meta: Record<string, unknown> }} EvidenceRecord */
...
/** @param {string} type @param {string} producer @param {unknown} content @param {EvidenceExtra} [extra] @returns {EvidenceRecord} */
function createEvidence(type, producer, content, extra = {}) {
  ...
  return {
    id: extra.id || `${normalizedType}:${digestContent({ producer: normalizedProducer, content }).slice(0, 16)}`,
    type: normalizedType,
