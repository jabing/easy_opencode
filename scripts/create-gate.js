...
const { createGateScaffold } = require('../src/cli/scaffold/gate.js');
...
function usage() {
  console.log('Usage: node scripts/create-gate.js <name> [summary] [--root <repo>]');
}
...
function main() {
  ...
  const summary = String(positionals.slice(1).join(' ') || `Scaffold gate ${name}.`).trim();
  const result = createGateScaffold(flags.root, { name, summary });
