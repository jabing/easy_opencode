...
const { createRunnerScaffold } = require('../src/cli/scaffold/runner.js');
...
function usage() {
  console.log('Usage: node scripts/create-runner.js <name> [summary] [--root <repo>]');
}
...
function main() {
  ...
  const summary = String(positionals.slice(1).join(' ') || `Scaffold runner ${name}.`).trim();
  const result = createRunnerScaffold(flags.root, { name, summary });
